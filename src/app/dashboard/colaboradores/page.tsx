...
export default async function CollaboratorsPage({
  ...
  return (
    <div className="space-y-6">
      ...
      <form className="no-print flex flex-col gap-3 rounded-3xl border border-line bg-background p-4 shadow-sm md:flex-row md:items-center">
        <input
          type="search"
          name="q"
          defaultValue={query}
        ...
        <button
          type="submit"
          className="rounded-2xl bg-panel-strong px-5 py-3 text-sm font-semibold text-white"
        >
          Buscar
        </button>
      </form>
          ...
          <article key={item.employee.id} className="print-card rounded-[2rem] border border-line bg-background p-6 shadow-sm">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              ...
              <div className="rounded-3xl bg-panel px-4 py-3 text-right">
                ...
                <p className="mt-1 text-2xl font-semibold text-foreground">{currency(item.netPay)}</p>
              </div>
            </div>
                  ...
                  {item.incomes
                    ...
                    .map((income) => (
                      <li key={income.label} className="flex items-center justify-between gap-3">
                        ...
                        <span className="font-medium text-foreground">{currency(income.amount)}</span>
                      </li>
                    ))}
                </ul>
              </div>
                ...
                <ul className="mt-3 space-y-2 text-sm text-muted">
                  {item.deductions.map((deduction) => (
                    <li key={deduction.label} className="flex items-center justify-between gap-3">
                      ...
                      <span className="font-medium text-foreground">{currency(deduction.amount)}</span>
                    </li>
                  ))}
                </ul>
              </div>
